...
int handle_render_mode(ArgParseState *state, char **argv, int argc, int *i_ptr) {
    AppState *app = (AppState *)state;
    const char *mode_str;
    if (!argparse_parse_string(argv, argc, i_ptr, &mode_str, "render-mode")) return 0;

    if (_stricmp(mode_str, "opengl") == 0) {
        app->render_mode = 1;
    } else if (_stricmp(mode_str, "software") == 0) {
        app->render_mode = 0;
    } else {
        fprintf(stderr, "ERROR: Invalid render mode '%s'. Must be 'software' or 'opengl'.\n", mode_str);
        return 0;
    }
    return 1;
}
...
ArgOption option_table[] = {
    ...
    {"--render", "--render-mode", handle_render_mode, "Specify render mode ('software' or 'opengl') <val>", 0, 0},
...
void update_window_title(HWND hWnd) {
    ...
    if (g_render_mode == 1) {
        strcpy(render_str, " (OpenGL)");
    } else {
        strcpy(render_str, " (Software)");
    }
    ...
    if (g_is_drawing_mode) {
        ...
        sprintf(mode_str, "Draw Mode ON (Tool: %s, Size: %d, Color: #%06X%s, Objects: %d)%s", 
            tool_name, 
            g_draw_size, 
        ...
        sprintf(mode_str, "Mode: %s, %.0f%% (Objects: %d, Sel: %d)%s", 
            get_mode_name(g_draw_mode), g_scale * 100.0f, g_drawing_count, g_selected_object_index, render_str
        );
    }
...
void upload_image_texture(pngl_uc *data, int w, int h) {
    if (g_texture_id != 0) {
...
int load_image_from_path(HWND hWnd, const char *path) {
    ...
    if (success) {
        ...
        if (g_render_mode == 1) {
             HDC hdc = GetDC(hWnd);
             upload_image_texture(g_image_data, g_width, g_height);
             ReleaseDC(hWnd, hdc);
        }
...
void draw_quad(float x, float y, float w, float h, float tx0, float ty0, float tx1, float ty1) {
    glBegin(GL_QUADS);
    glTexCoord2f(tx0, ty1); glVertex2f(x, y);
    glTexCoord2f(tx1, ty1); glVertex2f(x + w, y);
    glTexCoord2f(tx1, ty0); glVertex2f(x + w, y + h);
    glTexCoord2f(tx0, ty0); glVertex2f(x, y + h);
    glEnd();
}

void draw_background_opengl(int mode, int w, int h) {
    u32 color_u32;
    float r, g, b;
    int x, y;
    int tile_size = 16;
    ...
    if (mode == 0 || mode == 9) {
        ...
        for (x = 0; x < w; x += tile_size) {
            for (y = 0; y < h; y += tile_size) {
                ...
                r = (float)((color_u32_tile >> 16) & 0xFF) / 255.0f;
                g = (float)((color_u32_tile >> 8) & 0xFF) / 255.0f;
                b = (float)(color_u32_tile & 0xFF) / 255.0f;

                glColor3f(r, g, b);
                draw_quad((float)x, (float)y, (float)tile_size, (float)tile_size, 0.0f, 0.0f, 1.0f, 1.0f);
            }
        }
        
    } else {
        color_u32 = get_bg_color(mode);
        r = (float)((color_u32 >> 16) & 0xFF) / 255.0f;
...
void render_opengl(HWND hWnd) {
        ...
        switch (g_draw_mode) {
            ...
            case 1: /* Tiled / Repeat */
                if (scaled_width > 0 && scaled_height > 0) {
                    ...
                    float screen_h_total = (float)(tile_y_end - tile_y_start) * (float)scaled_height;

                    draw_quad(screen_x_start, screen_y_start, screen_w_total, screen_h_total, 0.0f, 0.0f, tx_end, ty_end);
    ...
    if (g_selected_object_index != -1 && g_is_drawing_mode == 0) {
        ...
        glVertex2f((float)draw_x - 2, (float)draw_y - 2);
        glVertex2f((float)draw_x + (float)draw_w + 2, (float)draw_y - 2);
        glVertex2f((float)draw_x + (float)draw_w + 2, (float)draw_y + (float)draw_h + 2);
        glVertex2f((float)draw_x - 2, (float)draw_y + (float)draw_h + 2);
        glEnd();
    }
...
void init_opengl(HWND hWnd, HDC hdc) {
...
void cleanup_opengl(HWND hWnd, HDC hdc) {
    if (g_texture_id != 0) {
        glDeleteTextures(1, &g_texture_id);
        g_texture_id = 0;
    }
...
void save_view_to_file(HWND hWnd) {
    ...
    if (g_framebuffer == NULL || w == 0 || h == 0) return;
    ...
    if (GetSaveFileName(&ofn)) {
        pngl_uc *data_for_png = (pngl_uc *)malloc((size_t)w * h * 4);
        if (data_for_png != NULL) {
            
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    u32 src = g_framebuffer[y * w + x];
                    pngl_uc *dst = data_for_png + y * w * 4 + x * 4;
                    
                    dst[0] = (pngl_uc)(src & 0xFF); 
                    dst[1] = (pngl_uc)((src >> 8) & 0xFF);
                    dst[2] = (pngl_uc)((src >> 16) & 0xFF);
                    dst[3] = 255; 
                }
            }
            
            if (pngl_write_png(ofn.lpstrFile, w, h, 4, data_for_png, 0)) {
            } else {
            }
...
void flood_fill_to_buffer(int x, int y, COLORREF new_color, pngl_uc *buffer) {
...

    {
        ...
        while (top > 0) {
            ...
            if (index + 3 >= (size_t)g_fb_w * g_fb_h * 4) continue; 
...
void draw_circle_to_buffer_point(int cx, int cy, int radius, COLORREF color, int alpha, pngl_uc *buffer) {
...
void draw_line_segment_to_buffer(int x0, int y0, int x1, int y1, COLORREF color, int radius, pngl_uc *buffer) {
    ...
    while (1) {
        draw_circle_to_buffer_point(x0, y0, radius, color, alpha, buffer);
...
void draw_circle_to_buffer_shape(int x0, int y0, int x1, int y1, COLORREF fill, COLORREF outline, int radius, int is_filled, int is_outlined, pngl_uc *buffer) {
    ...
    if (is_outlined) {
        ...
        while(x >= y) {
            draw_circle_to_buffer_point(cx + x, cy + y, pen_radius, outline, 255, buffer);
            draw_circle_to_buffer_point(cx + y, cy + x, pen_radius, outline, 255, buffer);
            draw_circle_to_buffer_point(cx - y, cy + x, pen_radius, outline, 255, buffer);
            draw_circle_to_buffer_point(cx - x, cy + y, pen_radius, outline, 255, buffer);
            draw_circle_to_buffer_point(cx - x, cy - y, pen_radius, outline, 255, buffer);
            draw_circle_to_buffer_point(cx - y, cy - x, pen_radius, outline, 255, buffer);
