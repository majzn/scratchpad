...
static void draw_panel(Rect r, const char *title, int active, int fill_char) {
  ...
  if (r.x < 0 || r.y < 0 || r.x + r.w > term_w || r.y + r.h > term_h)
    return;
...
static void draw_progress_v(int x, int y, int h, double val, int color_low,
  ...
  for (int i = 0; i < h; i++) {
    ...
    if (i < fill_h) {
      ...
      mzn_term_set_char(x, draw_y, '|',
                        MZN_ATTR_FG_BLACK | MZN_ATTR_BG_BLACK | MZN_ATTR_BOLD);
    }
  }
}
  ...
  if (f) {
    ...
    if (content) {
      ...
      if (cr_eval(engine, content, 1)) {
        ...
        char buf[64];
        for (int i = 0; i < 8; i++) {
          sprintf(buf, "t%d = %d", i + 1, triggers[i]);
          cr_eval(engine, buf, 0);
        }
      } else
        tui_log("Load Failed");
      free(content);
...
static void editor_load_file(const char *filename) {
  ...
  if (f) {
    ...
    while (fgets(buf, EDITOR_MAX_COL, f) &&
           editor_line_count < EDITOR_MAX_LINES) {
      int len = strlen(buf);
...
static void draw_popup_window(int mode) {
  ...
  for (int j = 0; j < h; j++)
    for (int i = 0; i < w; i++)
      mzn_term_set_char(x + i, y + j, ' ', fill);
  for (int i = 1; i < w - 1; i++) {
    mzn_term_set_char(x + i, y, '-', border);
    mzn_term_set_char(x + i, y + h - 1, '-', border);
  }
  for (int i = 1; i < h - 1; i++) {
    mzn_term_set_char(x, y + i, '|', border);
    mzn_term_set_char(x + w - 1, y + i, '|', border);
  }
  mzn_term_set_char(x, y, '+', border);
  mzn_term_set_char(x + w - 1, y, '+', border);
  mzn_term_set_char(x, y + h - 1, '+', border);
  mzn_term_set_char(x + w - 1, y + h - 1, '+', border);

  int tx = x + 3;
  int ty = y + 2;
  if (mode == 1) {
    mzn_term_draw_string(tx, ty, "NO FILE LOADED", fill | MZN_ATTR_BOLD);
    mzn_term_draw_string(tx, ty + 2, "Create New File? [Y/N]",
                         fill | MZN_ATTR_FG_YELLOW);
  } else if (mode == 2) {
    ...
    mzn_term_set_char(tx + 2 + popup_input_pos, ty + 2, '_',
                      MZN_ATTR_BG_WHITE | MZN_ATTR_FG_BLACK);
  } else if (mode == 3) {
    mzn_term_draw_string(tx, ty, "FILE EXISTS!", fill | MZN_ATTR_BOLD);
    mzn_term_draw_string(tx, ty + 2, "Overwrite? [Y/N]",
                         fill | MZN_ATTR_FG_YELLOW);
  }
}
...
void audio_cb(short *buf, int frames, int sr, int ch, void *data) {
  ...
  short *rec_buf = NULL;
  if (is_recording && rec_file)
    rec_buf = (short *)malloc(frames * 2 * sizeof(short));
  ...
  for (i = 0; i < frames; i++) {
    ...
    if (fft_idx < FFT_SIZE) {
      ...
      for (int k = 0; k < FFT_SIZE; k++) {
        fft_out[k].r = fft_in[k];
        fft_out[k].i = 0.0f;
      }
      simple_fft(fft_out, FFT_SIZE);

      waterfall_head =
          (waterfall_head - 1 + WATERFALL_HISTORY) % WATERFALL_HISTORY;

      for (int k = 0; k < FFT_SIZE / 2; k++) {
        float mag = cpx_mag(fft_out[k]);
        float log_mag = log10f(mag + 1.0f) * 2.0f;
        waterfall_buf[waterfall_head][k] =
            (waterfall_buf[waterfall_head][k] * WATERFALL_SMOOTHING) +
            (log_mag * (1.0f - WATERFALL_SMOOTHING));
      }
      fft_idx = 0;
    }
  ...
  peak_l = (pk_l > peak_l) ? pk_l : peak_l * 0.95;
...
static int get_ui_params(cr_context *ctx, ui_param_t *out_params, int max_p) {
  ...
  for (int i = 0; i < ctx->var_count && count < max_p; i++) {
    if (ctx->variables[i].name[0] == 't' &&
        isdigit(ctx->variables[i].name[1]) && ctx->variables[i].name[2] == 0)
      continue;
    ...
    if (n->op_desc && strcmp(n->op_desc->name, "param") == 0) {
      ...
      int input_idx = n->inputs[0];
      out_params[count].input_const_node_idx = input_idx;
      out_params[count].value = ctx->node_pool[input_idx].value.as.f;
      count++;
    }
  }
  return count;
...
static void editor_handle_input(int key) {
  ...
  if (key == K_UP) {
    ...
    if (ed_cy < editor_line_count - 1)
      ed_cy++;
  } else if (key == K_LEFT) {
    if (ed_cx > 0)
      ed_cx--;
  } else if (key == K_RIGHT) {
    if (ed_cx < (int)strlen(editor_lines[ed_cy]))
      ed_cx++;
  }
  ...
  if (key == K_ENTER) {
    if (editor_line_count < EDITOR_MAX_LINES - 1) {
      for (int i = editor_line_count; i > ed_cy + 1; i--)
    ...
    if (ed_cx > 0) {
      memmove(&editor_lines[ed_cy][ed_cx - 1], &editor_lines[ed_cy][ed_cx],
              len - ed_cx + 1);
      ed_cx--;
    } else if (ed_cy > 0) {
      ...
      if (prev_len + len < EDITOR_MAX_COL) {
        ...
        for (int i = ed_cy; i < editor_line_count - 1; i++)
          strcpy(editor_lines[i], editor_lines[i + 1]);
        editor_line_count--;
        ed_cy--;
        ed_cx = prev_len;
...
static void add_history(const char *line) {
  ...
  if (cmd_history_count > 0 &&
      !strcmp(cmd_history[cmd_history_count - 1], line))
    return;
    ...
    for (int bin = 0; bin < FFT_SIZE / 2; bin += 2) {
      ...
      if (screen_x >= r.x && screen_x < r.x + r.w) {
        ...
        if (peak_y < horizon[screen_x]) {
          char c = (i == 0) ? '#' : (i < 5 ? '=' : '-');
          int color = (i == 0) ? MZN_ATTR_FG_WHITE
                               : (i < 8 ? MZN_ATTR_FG_CYAN : MZN_ATTR_FG_BLUE);

          if (peak_y >= r.y) {
            mzn_term_set_char(screen_x, peak_y, c, color);
          }

          for (int y = peak_y + 1; y < horizon[screen_x]; y++) {
            if (y >= r.y && y < r.y + r.h) {
              mzn_term_set_char(screen_x, y, ' ', MZN_ATTR_BG_BLACK);
            }
          }
...
static void draw_modern_ui(const char *buf, int pos) {
  ...
  mzn_term_draw_string(2, 0, " CHRONOS v1.5 ",
                       MZN_ATTR_BG_BLUE | MZN_ATTR_FG_WHITE | MZN_ATTR_BOLD);

  char status[128];
  sprintf(status, " BPM: %.0f | CPU: %.1f%% ", engine->active->transport.bpm,
          cpu_load);
  mzn_term_draw_string(18, 0, status, MZN_ATTR_FG_CYAN);

  if (is_recording)
    mzn_term_draw_string(40, 0, " REC ",
                         MZN_ATTR_BG_RED | MZN_ATTR_FG_WHITE | MZN_ATTR_BOLD);

  if (loaded_filename[0]) {
    char fn[64];
    sprintf(fn, " FILE: %s ", loaded_filename);
    if ((int)strlen(fn) < w - 45)
      mzn_term_draw_string(w - strlen(fn) - 2, 0, fn, MZN_ATTR_FG_YELLOW);
  }
  ...
  if (scope_h < 10)
    scope_h = 10;
  if (scope_h > workspace_h - 6)
    scope_h = workspace_h - 6;

  Rect r_main_frame = {0, 1, main_w, workspace_h};
  Rect r_scope = {1, 2, main_w - 2, scope_h};
  Rect r_log = {1, 2 + scope_h, main_w - 2, workspace_h - (scope_h + 1)};
  Rect r_sidebar = {main_w, 1, sidebar_w, workspace_h};
  Rect r_footer = {0, h - footer_h, w, footer_h};

  if (ui_mode == 3) {
    draw_panel(r_main_frame, "EDITOR", 1, ' ');
    int viewport_h = r_main_frame.h - 2;
    if (ed_cy < ed_scroll)
      ed_scroll = ed_cy;
    if (ed_cy >= ed_scroll + viewport_h)
      ed_scroll = ed_cy - viewport_h + 1;

    for (int i = 0; i < viewport_h; i++) {
      int line_idx = ed_scroll + i;
      if (line_idx < editor_line_count) {
        char lnum[8];
        sprintf(lnum, "%4d ", line_idx + 1);
        mzn_term_draw_string(r_main_frame.x + 1, r_main_frame.y + 1 + i, lnum,
                             MZN_ATTR_BG_BLACK | MZN_ATTR_FG_MAGENTA);
        mzn_term_draw_string(r_main_frame.x + 6, r_main_frame.y + 1 + i,
                             editor_lines[line_idx], THEME_FG);
      }
    }
    int sx = r_main_frame.x + 6 + ed_cx;
    int sy = r_main_frame.y + 1 + (ed_cy - ed_scroll);
    char c = (ed_cx < (int)strlen(editor_lines[ed_cy]))
                 ? editor_lines[ed_cy][ed_cx]
                 : ' ';
    if (sx >= r_main_frame.x && sx < r_main_frame.x + r_main_frame.w - 1 &&
        sy >= r_main_frame.y && sy < r_main_frame.y + r_main_frame.h - 1)
      mzn_term_set_char(sx, sy, c, MZN_ATTR_BG_WHITE | MZN_ATTR_FG_BLACK);

    if (ed_msg[0])
      mzn_term_draw_string(r_main_frame.x + r_main_frame.w - strlen(ed_msg) - 2,
                           r_main_frame.y, ed_msg, THEME_ACCENT);

  } else {
    draw_panel(r_main_frame, "CONSOLE", ui_mode == 0, ' ');

    mzn_term_draw_string(r_scope.x + 1, r_scope.y - 1, "VISUALIZER",
                         THEME_MUTED);

    if (vis_mode == 0) {
      int center_y = r_scope.y + (r_scope.h / 2);
      for (int i = 0; i < r_scope.w; i++) {
        int idx = (waveform_head + i * (WAVEFORM_BUFFER_SIZE / r_scope.w)) %
                  WAVEFORM_BUFFER_SIZE;
        int y_l = center_y - (int)(waveform_l[idx] * 4);
        int y_r = center_y - (int)(waveform_r[idx] * 4);
        if (y_l < r_scope.y)
          y_l = r_scope.y;
        if (y_l >= r_scope.y + r_scope.h)
          y_l = r_scope.y + r_scope.h - 1;
        if (y_r < r_scope.y)
          y_r = r_scope.y;
        if (y_r >= r_scope.y + r_scope.h)
          y_r = r_scope.y + r_scope.h - 1;

        mzn_term_set_char(r_scope.x + i, y_l, '*', MZN_ATTR_FG_GREEN);
        if (y_r != y_l)
          mzn_term_set_char(r_scope.x + i, y_r, '*', MZN_ATTR_FG_RED);
      }
    } else if (vis_mode == 1) {
      int cx = r_scope.x + (r_scope.w / 2);
      ...
      for (int i = 0; i < WAVEFORM_BUFFER_SIZE; i += 30) {
        ...
        if (px >= r_scope.x && px < r_scope.x + r_scope.w && py >= r_scope.y &&
            py < r_scope.y + r_scope.h)
          mzn_term_set_char(px, py, '.', MZN_ATTR_FG_CYAN);
      }
    } else if (vis_mode == 2) {
      mzn_term_draw_string(r_scope.x + 1, r_scope.y, "SPECTRAL TERRAIN",
                           THEME_MUTED);
      draw_waterfall_3d(r_scope);
    }

    for (int i = 1; i < r_main_frame.w - 1; i++)
      mzn_term_set_char(r_main_frame.x + i, r_log.y - 1, '-', THEME_BORDER);
    mzn_term_draw_string(r_log.x + 1, r_log.y - 1, "LOG", THEME_MUTED);

    int max_vis = r_log.h;
    int max_scroll =
        (log_history_count > max_vis) ? log_history_count - max_vis : 0;
    ...
    for (int i = 0; i < max_vis && start + i < log_history_count; i++) {
      char lbuf[1024];
      strncpy(lbuf, log_history[start + i], r_log.w - 1);
      lbuf[r_log.w - 1] = 0;
      int attr = THEME_FG;
      if (strstr(lbuf, "[ERR]"))
        attr = MZN_ATTR_FG_RED;
      else if (strstr(lbuf, "[WRN]"))
        attr = MZN_ATTR_FG_YELLOW;
      else if (strstr(lbuf, "[INF]"))
        attr = MZN_ATTR_FG_CYAN;
  ...
  if (sidebar_w > 0) {
    draw_panel(r_sidebar, "CONTROLS", ui_mode == 1, ' ');

    int ty = r_sidebar.y + 1;
    mzn_term_draw_string(r_sidebar.x + 2, ty++, "TRIGGERS (1-8)", THEME_TITLE);
    for (int i = 0; i < 2; i++) {
      char tbuf[64];
      sprintf(tbuf, "%d:%s %d:%s %d:%s %d:%s", i * 4 + 1,
              triggers[i * 4 + 0] ? "ON " : " . ", i * 4 + 2,
              triggers[i * 4 + 1] ? "ON " : " . ", i * 4 + 3,
              triggers[i * 4 + 2] ? "ON " : " . ", i * 4 + 4,
              triggers[i * 4 + 3] ? "ON " : " . ");
      mzn_term_draw_string(r_sidebar.x + 2, ty++, tbuf, THEME_FG);
    }

    ty += 1;
    mzn_term_draw_string(r_sidebar.x + 2, ty++, "OUTPUT", THEME_TITLE);
    mzn_term_draw_string(r_sidebar.x + 2, ty + 6, "L   R", THEME_MUTED);
    draw_progress_v(r_sidebar.x + 2, ty, 6, peak_l, MZN_ATTR_FG_GREEN,
                    MZN_ATTR_FG_RED);
    draw_progress_v(r_sidebar.x + 6, ty, 6, peak_r, MZN_ATTR_FG_GREEN,
                    MZN_ATTR_FG_RED);
    ty += 8;

    mzn_term_draw_string(r_sidebar.x + 2, ty++, "PARAMS (TAB)", THEME_TITLE);
    ui_param_t params[32];
    int p_count = get_ui_params(engine->active, params, 32);
    if (selected_param_idx >= p_count)
      selected_param_idx = (p_count > 0 ? p_count - 1 : 0);

    int max_params_h = r_sidebar.h - (ty - r_sidebar.y) - 1;
    int p_start = 0;
    if (selected_param_idx >= max_params_h)
      p_start = selected_param_idx - max_params_h + 1;

    for (int i = 0; i < max_params_h && (p_start + i) < p_count; i++) {
      int idx = p_start + i;
      int is_sel = (ui_mode == 1 && idx == selected_param_idx);
      char pbuf[64];
      sprintf(pbuf, "%-14s %6.3f", params[idx].name, params[idx].value);
      int attr = is_sel ? (MZN_ATTR_BG_CYAN | MZN_ATTR_FG_BLACK) : THEME_FG;
      mzn_term_draw_string(r_sidebar.x + 2, ty++, pbuf, attr);
    }
  }

  for (int i = 0; i < w; i++)
    mzn_term_set_char(i, r_footer.y, '-', THEME_BORDER);

  if (ui_mode == 2) {
    mzn_term_draw_string(
        2, r_footer.y + 1,
        " HELP: E=Edit, R=Rec, V=Vis, TAB=Focus, ESC=Panic, Ctrl+S=Save ",
        MZN_ATTR_FG_YELLOW);
  } else {
    mzn_term_draw_string(2, r_footer.y + 1,
                         (ui_mode == 0) ? "CMD > " : "    > ",
                         (ui_mode == 0) ? MZN_ATTR_FG_GREEN : THEME_MUTED);
    if (ui_mode == 0) {
      mzn_term_draw_string(8, r_footer.y + 1, buf, MZN_ATTR_FG_WHITE);
      mzn_term_set_char(8 + pos, r_footer.y + 1, buf[pos] ? buf[pos] : ' ',
                        MZN_ATTR_BG_WHITE | MZN_ATTR_FG_BLACK);
    } else {
      mzn_term_draw_string(8, r_footer.y + 1,
                           "[PARAMETER MODE] Use Arrows to adjust values",
                           THEME_MUTED);
    }
  }

  mzn_term_draw_string(w - 20, r_footer.y + 1, "Press 'H' for Help",
                       THEME_MUTED);

  if (popup_mode > 0)
    draw_popup_window(popup_mode);
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    if (ui_mode != 3 && popup_mode == 0 && loaded_filename[0] &&
        now > last_check_time) {
      last_check_time = now;
      struct stat s;
      if (stat(loaded_filename, &s) == 0) {
        if (s.st_mtime > last_mod_time && now - s.st_mtime >= 1)
          load_script(loaded_filename);
      }
    }
    ...
    if (key != -1) {
      if (popup_mode > 0) {
        handle_popup_input(key);
        continue;
      }
      if (ui_mode == 3) {
        editor_handle_input(key);
        continue;
      }
      if (key == K_TAB) {
        if (ui_mode == 2)
      ...
      if (key == K_ESC) {
        cr_eval(engine, "out = 0", 0);
        tui_log("PANIC: Output muted.");
        continue;
      }
      ...
      if ((key == 'e' || key == 'E') && ui_mode != 0) {
        if (loaded_filename[0] || editor_line_count > 0) {
          if (loaded_filename[0])
            editor_load_file(loaded_filename);
          ui_mode = 3;
          continue;
        }
      }
      if ((key == 'h' || key == 'H' || key == '?') && ui_mode != 0) {
        if (ui_mode == 2)
          ui_mode = 0;
        else
          ui_mode = 2;
      ...
      if (key == K_PGUP) {
        log_scroll_offset += 5;
        if (log_scroll_offset > MAX_LOG_LINES)
          log_scroll_offset = MAX_LOG_LINES;
      } else if (key == K_PGDN) {
        log_scroll_offset -= 5;
        if (log_scroll_offset < 0)
          log_scroll_offset = 0;
      ...
      int typing_command = (ui_mode == 0 && line_pos > 0);
      if (!typing_command) {
        if (key == 'r' || key == 'R') {
          toggle_recording();
          continue;
        ...
        if (key >= '1' && key <= '8') {
          int t_idx = key - '1';
          triggers[t_idx] = !triggers[t_idx];
          char buf[64];
          sprintf(buf, "t%d = %d", t_idx + 1, triggers[t_idx]);
          cr_eval(engine, buf, 0);
          continue;
        }
      }
      ...
      if (ui_mode == 1) {
        ...
        if (key == K_ENTER) {
          if (!strcmp(line_buf, "quit"))
            break;
          else if (strncmp(line_buf, "load ", 5) == 0) {
            load_script(line_buf + 5);
            editor_load_file(line_buf + 5);
            ...
            if (loaded_filename[0] || editor_line_count > 0)
              ...
              tui_log("No file to edit");
          } else if (len > 0) {
            add_history(line_buf);
            if (cr_eval(engine, line_buf, 0))
              tui_log("Executed.");
            else
              tui_log("Eval Error.");
          }
