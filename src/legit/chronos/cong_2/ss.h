...
typedef void (*ss_audio_cb_t)(short *buf, int n_frames, int s_rate, int ch,
                              void *udata);
...
ss_audio_t *ss_open_audio(ss_audio_cb_t play_cb, ss_audio_cb_t rec_cb,
                          int s_rate, int ch, int buf_frames, void *udata);
...
ss_thread_t ss_thread_create(ss_thread_func_t func, void *arg) {
  return (ss_thread_t)_beginthreadex(
      NULL, 0, (unsigned int(__stdcall *)(void *))func, arg, 0, NULL);
}
...
void ss_thread_join(ss_thread_t thread) { pthread_join(thread, NULL); }
...
static LRESULT CALLBACK ss_window_proc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                       LPARAM lParam) {
  ...
  case WM_SYSKEYDOWN:
    if (wParam < 256)
      wnd->input.key_st[wParam] = 1;
    return 0;
  case WM_KEYUP:
  case WM_SYSKEYUP:
    if (wParam < 256)
      wnd->input.key_st[wParam] = 0;
    return 0;
  case WM_LBUTTONDOWN:
    wnd->input.mouse_l = 1;
    return 0;
...
ss_window_t *ss_open_window(int w, int h, const char *title) {
  ...
  wnd->hwnd = CreateWindowEx(0, "SS_WNDCLASS", title, dwStyle, CW_USEDEFAULT,
...
void ss_close_window(ss_window_t *wnd) {
  if (wnd) {
    if (wnd->hdc_wnd)
      ReleaseDC(wnd->hwnd, wnd->hdc_wnd);
    if (wnd->hwnd)
...
void ss_blit_fb(ss_window_t *wnd, unsigned char *fb, int w, int h) {
  ...
  wnd->bmi.bmiHeader.biWidth = w;
  wnd->bmi.bmiHeader.biHeight = -h;
  StretchDIBits(wnd->hdc_wnd, 0, 0, wnd->w, wnd->h, 0, 0, w, h, fb, &wnd->bmi,
                DIB_RGB_COLORS, SRCCOPY);
}

ss_input_t ss_get_input(ss_window_t *wnd) { return wnd->input; }
...
static unsigned long __stdcall ss_was_thread(void *arg) {
  ...
  if (p_active)
    w_hndl[n_w_hndl++] = a->hEvt_p;
  if (r_active)
    w_hndl[n_w_hndl++] = a->hEvt_c;

  while (a->running) {
    w_res = WaitForMultipleObjects(n_w_hndl, w_hndl, FALSE, 500);

    if (w_res == WAIT_TIMEOUT)
      continue;

    if (p_active && (w_res == WAIT_OBJECT_0 ||
                     (r_active && w_res == (WAIT_OBJECT_0 + 1)))) {
    ...
    if (r_active && (w_res == WAIT_OBJECT_0 + (p_active ? 1 : 0))) {
      ...
      while (n_to_read > 0) {
        hr = IAudioCaptureClient_GetBuffer(a->pCC, &pData, &n_avail, &flags,
                                           NULL, NULL);
        if (FAILED(hr))
          break;
...
static HRESULT ss_was_init(struct ss_audio_internal *a, EDataFlow flow,
                           AUDCLNT_SHAREMODE mode, IAudioClient **ppAC,
                           UINT32 *pBufFrCount, HANDLE *phEvt,
                           void **ppClient) {
  ...
  hr = IMMDevice_Activate(pDev, &IID_IAudioClient, CLSCTX_ALL, NULL,
                          (void **)ppAC);
  if (FAILED(hr)) {
  ...
  if (a->s_rate != 44100) {
    fprintf(stderr,
            "[SS_AUDIO_WARN] System is using %d Hz Mix Rate. Chronos context "
            "will be updated.\n",
            a->s_rate);
  }
  fprintf(
      stderr,
      "[SS_AUDIO_DEBUG] Device Mix Format: Rate=%d Hz, Channels=%d, Bits=%d\n",
  ...
  wfx_candidate.nBlockAlign =
      wfx_candidate.nChannels * (wfx_candidate.wBitsPerSample / 8);
  wfx_candidate.nAvgBytesPerSec =
      wfx_candidate.nSamplesPerSec * wfx_candidate.nBlockAlign;
  wfx_candidate.cbSize = 0;
  hr = IAudioClient_IsFormatSupported(*ppAC, mode, &wfx_candidate, &pWfx_use);

  if (hr == S_OK) {
    fprintf(stderr, "[SS_AUDIO_DEBUG] 16-bit PCM format supported. Using "
                    "candidate format.\n");
    pWfx_use = &wfx_candidate;
  } else {
    fprintf(stderr,
            "[SS_AUDIO_DEBUG] 16-bit PCM not directly supported (HR: 0x%lX). "
            "Falling back to Mix Format.\n",
            hr);
    pWfx_use = pWfx_mix;
  }
  ...
  *phEvt = CreateEvent(NULL, FALSE, FALSE, NULL);
  ...
  hr = IAudioClient_Initialize(*ppAC, mode, AUDCLNT_STREAMFLAGS_EVENTCALLBACK,
                               hnsReqDur, 0, pWfx_use, NULL);

  CoTaskMemFree(pWfx_mix);
  if (pWfx_use != &wfx_candidate && pWfx_use != pWfx_mix)
    CoTaskMemFree(pWfx_use);
  if (FAILED(hr)) {
    fprintf(stderr,
            "[SS_AUDIO_ERROR] IAudioClient_Initialize failed: %s (0x%lX)\n",
            HResultToString(hr), hr);
    CloseHandle(*phEvt);
    *phEvt = NULL;
    IAudioClient_Release(*ppAC);
...
ss_audio_t *ss_open_audio(ss_audio_cb_t play_cb, ss_audio_cb_t rec_cb,
                          int s_rate, int n_ch, int buf_frames, void *udata) {
  struct ss_audio_internal *a;
  HRESULT hr;

  fprintf(
      stderr,
      "[SS_AUDIO_DEBUG] Entering ss_open_audio. Attempting allocation...\n");
  a = (struct ss_audio_internal *)malloc(sizeof(struct ss_audio_internal));
  if (!a) {
    fprintf(stderr,
            "[SS_AUDIO_ERROR] Failed to allocate ss_audio_internal memory.\n");
    return NULL;
  }
  ...
  fprintf(
      stderr,
      "[SS_AUDIO_DEBUG] Initializing COM with COINIT_APARTMENTTHREADED...\n");
  ...
  fprintf(stderr,
          "[SS_AUDIO_DEBUG] Creating IMMDeviceEnumerator instance...\n");
  hr = CoCreateInstance(&CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL,
                        &IID_IMMDeviceEnumerator, (void **)&a->pEnum);
  if (FAILED(hr)) {
    fprintf(stderr,
            "[SS_AUDIO_ERROR] CoCreateInstance IMMDeviceEnumerator failed: %s "
            "(0x%lX)\n",
            HResultToString(hr), hr);
    CoUninitialize();
    free(a);
  ...
  fprintf(stderr,
          "[SS_AUDIO_DEBUG] IMMDeviceEnumerator created successfully.\n");

  if (play_cb) {
    fprintf(
        stderr,
        "[SS_AUDIO_DEBUG] Getting Playback (eRender) default endpoint...\n");
    hr = IMMDeviceEnumerator_GetDefaultAudioEndpoint(a->pEnum, eRender,
                                                     eConsole, &a->pDev_p);
    if (FAILED(hr)) {
      fprintf(stderr,
              "[SS_AUDIO_ERROR] GetDefaultAudioEndpoint eRender failed: %s "
              "(0x%lX)\n",
              HResultToString(hr), hr);
    ...
    hr = ss_was_init(a, eRender, AUDCLNT_SHAREMODE_SHARED, &a->pAC_p,
                     &a->buf_fr_c_p, &a->hEvt_p, (void **)&a->pRC);
    if (FAILED(hr)) {
      fprintf(stderr, "[SS_AUDIO_ERROR] Playback initialization failed. Check "
                      "ss_was_init errors above.\n");
      ss_close_audio((ss_audio_t *)a);
      return NULL;
    }
  }
  ...
  if (rec_cb) {
    fprintf(
        stderr,
        "[SS_AUDIO_DEBUG] Getting Capture (eCapture) default endpoint...\n");
    hr = IMMDeviceEnumerator_GetDefaultAudioEndpoint(a->pEnum, eCapture,
                                                     eConsole, &a->pDev_c);
    if (FAILED(hr)) {
      fprintf(stderr,
              "[SS_AUDIO_ERROR] GetDefaultAudioEndpoint eCapture failed: %s "
              "(0x%lX)\n",
              HResultToString(hr), hr);
      ss_close_audio((ss_audio_t *)a);
      return NULL;
    ...
    hr = ss_was_init(a, eCapture, AUDCLNT_SHAREMODE_SHARED, &a->pAC_c,
                     &a->buf_fr_c_c, &a->hEvt_c, (void **)&a->pCC);
    if (FAILED(hr)) {
      fprintf(stderr, "[SS_AUDIO_ERROR] Capture initialization failed. Check "
                      "ss_was_init errors above.\n");
      ss_close_audio((ss_audio_t *)a);
      return NULL;
    }
  ...
  if (play_cb && a->pAC_p) {
    fprintf(stderr, "[SS_AUDIO_DEBUG] Starting Playback stream.\n");
    IAudioClient_Start(a->pAC_p);
  }
  if (rec_cb && a->pAC_c) {
    fprintf(stderr, "[SS_AUDIO_DEBUG] Starting Capture stream.\n");
...
void ss_close_audio(ss_audio_t *a) {
  ...
  if (internal_a->a_thread)
    ss_thread_join(internal_a->a_thread);

  if (internal_a->pAC_p)
    IAudioClient_Stop(internal_a->pAC_p);
  if (internal_a->pAC_c)
    IAudioClient_Stop(internal_a->pAC_c);

  if (internal_a->hEvt_p)
    CloseHandle(internal_a->hEvt_p);
  if (internal_a->hEvt_c)
    CloseHandle(internal_a->hEvt_c);

  if (internal_a->pRC)
    IAudioRenderClient_Release(internal_a->pRC);
  if (internal_a->pCC)
    IAudioCaptureClient_Release(internal_a->pCC);

  if (internal_a->pAC_p)
    IAudioClient_Release(internal_a->pAC_p);
  if (internal_a->pAC_c)
    IAudioClient_Release(internal_a->pAC_c);

  if (internal_a->pDev_p)
    IMMDevice_Release(internal_a->pDev_p);
  if (internal_a->pDev_c)
    IMMDevice_Release(internal_a->pDev_c);
  if (internal_a->pEnum)
    IMMDeviceEnumerator_Release(internal_a->pEnum);
...
ss_window_t *ss_open_window(int w, int h, const char *title) {
  ...
  cmap = XCreateColormap(wnd->disp, RootWindow(wnd->disp, wnd->scr),
  ...
  wnd->wnd = XCreateWindow(wnd->disp, RootWindow(wnd->disp, wnd->scr), 0, 0, w,
                           h, 0, depth, InputOutput, vinfo.visual,
  ...
  wnd->xim = XCreateImage(wnd->disp, vinfo.visual, depth, ZPixmap, 0,
...
void ss_close_window(ss_window_t *wnd) {
  if (wnd) {
    ...
    if (wnd->gc)
      XFreeGC(wnd->disp, wnd->gc);
    if (wnd->wnd)
      XDestroyWindow(wnd->disp, wnd->wnd);
    if (wnd->disp)
      XCloseDisplay(wnd->disp);
    free(wnd);
  }
}
...
static int ss_alsa_setup(snd_pcm_t **h, const char *dev, int acc, int s_rate,
                         int n_ch, snd_pcm_uframes_t *buf_frames) {
  ...
  if (rc < 0) {
    fprintf(stderr, "ALSA: SW params failed: %s\n", snd_strerror(rc));
    return rc;
  }
...
static void *ss_alsa_thread(void *arg) {
  ...
  while (a->running) {
    if (a->play_cb) {
      ...
      if (rc_p < 0 && rc_p != -EPIPE)
        break;
      if (rc_p == -EPIPE)
        snd_pcm_prepare(a->play_h);
    }

    if (a->rec_cb) {
      rc_c = snd_pcm_readi(a->rec_h, a->rec_buf, a->buf_frames);
      if (rc_c < 0 && rc_c != -EPIPE)
        break;
      if (rc_c == -EPIPE)
        snd_pcm_prepare(a->rec_h);
...
ss_audio_t *ss_open_audio(ss_audio_cb_t play_cb, ss_audio_cb_t rec_cb,
  ...
  if (play_cb) {
    if (ss_alsa_setup(&a->play_h, "default", SND_PCM_STREAM_PLAYBACK, s_rate,
                      n_ch, &alsa_frames) < 0) {
      ss_close_audio((ss_audio_t *)a);
      return NULL;
    }
  ...
  if (rec_cb) {
    if (ss_alsa_setup(&a->rec_h, "default", SND_PCM_STREAM_CAPTURE, s_rate,
                      n_ch, &alsa_frames) < 0) {
      ss_close_audio((ss_audio_t *)a);
      return NULL;
    }
...
void ss_close_audio(ss_audio_t *a) {
  ...
  if (internal_a->play_h) {
    ...
    if (internal_a->play_buf)
      free(internal_a->play_buf);
  }

  if (internal_a->rec_h) {
    snd_pcm_close(internal_a->rec_h);
    if (internal_a->rec_buf)
      free(internal_a->rec_buf);
  }
